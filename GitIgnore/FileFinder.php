...
class FileFinder
{
    ...
    protected function match($rootDir, $f)
    {
        ...
        if ($this->invertRulesetResult and $result !== null) {
            $result = !$result;
        }
    ...
    protected function _findFiles($rootDir, $f)
    {
        if (preg_match('#/$#', $rootDir)) {
            throw new Exception("Root directory argument to _findFiles should not end with a slash; given «{$rootDir}»");
        }
        if (preg_match('#^/|/$#', $f)) {
            throw new Exception("Relative path argument to _findFiles should not start or end with a slash; given «{$f}»");
        }
